...
for (pca_type in c("unfiltered", "filtered")) {
  ...
  ggsave(
    paste0("plots/pca/", pca_type, "/variance_explained.pdf"),
...
for (pca_type in c("unfiltered", "filtered")) {
  ...
  ggplot(pca_df[[pca_type]], aes(x = PC1, y = PC2, color = batch)) +
    geom_point()
  ggsave(paste0(pca_dir, "/by_batch.pdf"), width = 5, height = 3)

  ggplot(pca_df[[pca_type]], aes(x = PC1, y = PC2, color = RNA_predicted_sex)) +
    geom_point()
  ggsave(paste0(pca_dir, "/by_sex.pdf"), width = 5, height = 3)
  ...
  ggplot(
    ...
    geom_point()
  ggsave(paste0(pca_dir, "/by_batch_and_sex.pdf"), width = 5, height = 3)

  # sample name annotated
  ggplot(pca_df[[pca_type]], aes(x = PC1, y = PC2, color = batch)) +
    geom_point() +
    geom_label_repel(aes(label = paper_key),
      box.padding = unit(0.35, "lines"),
      point.padding = unit(0.5, "lines"),
      segment.color = "grey50"
    )
  ggsave(paste0(pca_dir, "/by_batch_annotated.pdf"), width = 5, height = 3)
  ...
  for (snp_name in SNP_names) {
    ...
    ggsave(paste0(pca_dir, "/", snp_name, ".pdf"), width = 5, height = 3)
  }
}
```
...
for (beta_df_output_dir in list(
  list(beta$unfiltered, "unfiltered/champ/all_probes"),
  list(beta$filtered, "filtered/champ/all_probes"),
  list(high_variance_beta$unfiltered, "unfiltered/champ/top_500_probes"),
  list(high_variance_beta$filtered, "filtered/champ/top_500_probes")
)) {
  beta_df <- beta_df_output_dir[[1]]
  output_dir <- beta_df_output_dir[[2]]
  if (!dir.exists(paste0("plots/pca/", output_dir))) {
    dir.create(paste0("plots/pca/", output_dir), recursive = TRUE)
  }
  champ.SVD(
    ...
    resultsDir = paste0("plots/pca/", output_dir, "/")
  )
}
```


As an alternative way of looking at the beta values, now for a heatmap over all
CpGs (columns) and samples (rows).
``` {r heatmap}
if (!dir.exists("plots/heatmap")) {
  dir.create("plots/heatmap", recursive = TRUE)
}
lapply(c("unfiltered", "filtered"), function(pca_type) {
  colnames(high_variance_beta[[pca_type]]) <- samplesheet[
    match(colnames(high_variance_beta[[pca_type]]), samplesheet$minfi_samplename),
    "paper_key"
  ]
  ## genotype colour sets
  geno_cols_C_G <- c("CC" = "#EE7733", "CG" = "#0077BB", "GG" = "#EE3377")
  geno_cols_C_T <- c("CC" = "#EE7733", "CT" = "#33BBEE", "TT" = "#009988")

  pdf(paste0("plots/heatmap/", pca_type, ".pdf"), width = 8.5, height = 5.5)
  Heatmap(
    t(high_variance_beta[[pca_type]]),
    name = "beta",
    col = colorRamp2(c(0, 0.5, 1), c("#313695", "#FFFFFF", "#A50026")),
    show_column_names = FALSE,
    column_dend_height = unit(2, "cm"),
    row_dend_width = unit(2, "cm"),
    left_annotation = rowAnnotation(
      df = samplesheet[
        match(colnames(high_variance_beta[[pca_type]]), samplesheet$paper_key),
        SNP_names
      ],
      col = list(
        PNPLA3 = geno_cols_C_G, TM6SF2 = geno_cols_C_T,
        TMC4 = geno_cols_C_T, HSD17B13 = geno_cols_C_G
      ),
      na_col = "grey"
    )
  ) %>%
...
First, we have to filter out the second sample of the repeated HSC16.
``` {r filter-repeats}
samplesheet <- samplesheet %>%
  filter(!(Sample_Name == "HSC16" & batch == "Batch 2"))
beta_without_repeats <- lapply(
  c(unfiltered = "unfiltered", filtered = "filtered"),
  function(type) {
    beta[[type]][, colnames(beta[[type]]) %in% samplesheet$minfi_samplename]
  }
)
message(paste0(
  "After removing repeated HSC16: ", ncol(beta_without_repeats$filtered), " samples (", ncol(beta$filtered) - ncol(beta_without_repeats$filtered), " removed)"
))
beta <- beta_without_repeats
```
...
if (!file.exists("meqtl_results/targeted_meQTL_raw.csv")) {
  if (!dir.exists("meqtl_results")) {
    dir.create("meqtl_results")
  }
  beta$filtered %>%
    melt(varnames = c("probe", "sample"), value.name = "beta_value") %>%
    merge(
...
comparison_names <- data.frame(
  ...
  name = c(
    "PNP_CGvCC", "PNP_GGvCC", "PNP_GGvCG",
    "TM6_CTvCC",
    "TMC_CTvCC", "TMC_TTvCC", "TMC_TTvCT",
    "HSD_CGvCC",
    "TM6_PNP_CC-CGvCC-CC", "TM6_PNP_CC-GGvCC-CC", "TM6_PNP_CC-GGvCC-CG",
    "PNP_TMC_CC-CTvCC-CC",
    "TMC_PNP_CT-CGvCT-CC",
    "TM6_TMC_CC-CTvCC-CC"
  )
...
meqtl_results %>%
  rename_with(~ paste0("probe_", .), starts_with("start")) %>%
  rename_with(~ paste0("probe_", .), starts_with("end")) %>%
  filter(Holm_p_value < 0.05) %>%
  mutate(
    comparison_name = factor(comparison, levels = comparison_names$comparison) %>%
      as.character() %>%
      factor(
        levels = comparison_names$comparison,
        labels = comparison_names$name
      )
  ) %>%
  left_join(
    gene_annotations[
      , c(
        "probe", "ensembl_gene_id", "hgnc_symbol",
        "entrezgene_id", "start_position", "end_position"
      )
    ] %>%
      rename_with(~ paste0("gene_", .), starts_with("start")) %>%
      rename_with(~ paste0("gene_", .), starts_with("end")),
    by = "probe"
  ) %>%
  write_csv("meqtl_results/significant_results.csv")
```

Finally, let's plot the beta value for each significant result, with the gene on the
x axis and the beta value on the y axis.
``` {r plot-significant-results}
...
significant_results %>%
  ...
  dplyr::select(
    probe, comparison, comparison_name, hgnc_symbol, beta_value, sample
  ) %>%
  merge(
    comparison_group_memberships,
    how = "left",
    by.y = c("minfi_samplename", "comparison"), by.x = c("sample", "comparison")
  ) %>%
  mutate(
    genotype = ifelse(
      group == 0, # then extract the first genotype
      # split on _, take the second, then split on v and take the first
      comparison_name %>%
        str_split("_") %>%
        map_chr(-1) %>%
        str_split("v") %>%
    ...
    annotated_probe = paste0(
      probe, " (",
      ifelse(is.na(hgnc_symbol), "No Gene", hgnc_symbol), ")"
    )
  ) %>%
  ggplot(aes(x = genotype, y = beta_value)) +
  geom_point() +
  geom_jitter(width = 0.2, height = 0) +
  facet_wrap(~ comparison_name + annotated_probe, scales = "free_x", ncol = 5) +
  ylim(0, 1)
if (!dir.exists("plots/meqtl")) {
  dir.create("plots/meqtl")
}
ggsave("plots/meqtl/significant_result_beta.pdf", width = 9.5, height = 5)
```

``` {r plot-significant-results-all}
significant_results %>%
  merge(
    beta$filtered %>%
      melt(varnames = c("probe", "sample"), value.name = "beta_value"),
  ...
  mutate(
    relevant_SNP = ifelse(
      grepl("TMC_PNP", comparison_name),
      NA,
      comparison_name %>%
        str_extract("PNP|TMC|TM6|HSD")
    )
  ) %>%
  merge(
    samplesheet %>% # add the SNP genotype to the data
      dplyr::select(all_of(c("minfi_samplename", SNP_names))) %>%
      pivot_longer(cols = -minfi_samplename, names_to = "SNP", values_to = "genotype") %>%
      mutate(
        abbreviated_SNP = str_extract(SNP, "PNP|TMC|TM6|HSD")
      ),
    by.x = c("sample", "relevant_SNP"),
    by.y = c("minfi_samplename", "abbreviated_SNP")
  ) %>%
  mutate(
    annotated_probe = paste0(
      probe, " (",
  ...
  geom_point() +
  geom_jitter(width = 0.2, height = 0) +
  facet_wrap(~ SNP + annotated_probe, scales = "free_x", ncol = 5) +
  ylim(0, 1)
ggsave("plots/meqtl/significant_result_beta_all.pdf", width = 8, height = 5, create.dir = TRUE)
```
