...
outlier_minfi_samplenames <- c(
    "206053660012_R08C01", "206693200099_R08C01", "206053660068_R07C01"
)
beta_without_outliers <- lapply(
    c(unfiltered = "unfiltered", filtered = "filtered"),
    function(type) {
        beta[[type]][, !(colnames(beta[[type]]) %in% outlier_minfi_samplenames)]
    }
)
beta <- beta_without_outliers
...
As an alternative way of looking at the beta values, go for a heatmap.
``` {r heatmap}
...
lapply(c("unfiltered", "filtered"), function(pca_type) {
    ...
    png(paste0("plots/heatmap/", pca_type, ".png"), width = 600, height = 400)
    Heatmap(
        ...
        left_annotation = rowAnnotation(
            df = samplesheet[
                match(colnames(high_variance_beta[[pca_type]]), samplesheet$Sample_Name),
                SNP_names
            ],
            col = list(
...
``` {r heatmap-legend}
# now save a png of a legend for CC,CG,GG and for CC,CT,TT
png(paste0("plots/heatmap/legend_GT.png"), width = 230, height = 280)
par(mar = c(0, 0, 0, 0))
plot(1, type = "n", axes = FALSE, xlab = "", ylab = "")
legend(
    "center",
    legend = c("CC", "CG/CT", "GG/TT", "NA"),
    fill = c("red", "blue", "green", "grey"),
    title = "Genotype", cex = 3
)
dev.off()
```
...
Final bit, let's plot the beta value for each significant result, with the gene on the
...
Similar to the last plot, but include all values for the genotype.
Separate facet for each SNP-probe pair which has any significant results. (Eg include 
(TM6, cg02534048) because it has significant results for TM6_CCvCT). Now x-axis should
be CC,CT,TT for TM6, and so on.
